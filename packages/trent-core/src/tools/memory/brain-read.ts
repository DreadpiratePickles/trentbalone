/**
 * [C2] `brain_read {"path": "<path>"}` — the tool that turns a signpost into a body.
 *
 * The stable tier lists the brain's file tree as paths and nothing else: the bodies of `memory/`,
 * `decisions/` and `seats/` stay out of the prompt until an objective needs one. This adapter is
 * how a seat fetches one, and it is deliberately the smallest tool in the fleet: one argument, no
 * write path, no approval, no network.
 *
...
const ROUTING_TEXT =
  ...
  "open a file listed in the brain tree, seat notes";

export const BRAIN_READ_TOOL_SCHEMAS: ToolSchema[] = [
  {
    name: "brain_read",
    description:
      "Read one file from the company brain by the path listed in your prompt's brain file tree " +
      "(for example decisions/2026-09-18-the-decision.md or memory/2026-09-18.md). Read-only, and " +
      "only inside the brain: the standing decisions and episodic notes are kept out of the prompt " +
      "until you ask for one.",
    parameters: {
      type: "object",
      properties: { path: { type: "string", description: "A path relative to the brain root, exactly as the file tree lists it." } },
      required: ["path"],
    },
  },
];
...
export function createBrainReadAdapter(options: BrainReadAdapterOptions): TrentToolAdapter {
  ...
  return {
    ...
    async execute(action) {
      ...
      if (requested === "") return record(action, "failed", 'brain_read requires "path": one of the paths listed in the brain file tree.');

      let absolute: string;
      try {
        absolute = resolveBrainPath(options.profileDir, requested);
      } catch (err) {
        return record(action, "blocked", `brain_read refused "${requested}": ${(err as Error).message}. Paths are relative to the brain root.`);
      }
      ...
      try {
        ...
        return record(action, "failed", `brain_read: there is no file at "${requested}". The brain file tree in your prompt lists what there is.`);
      }
      if (!insideBrain(absolute)) {
        return record(action, "blocked", `brain_read refused "${requested}": it resolves outside the brain directory.`);
      }
      if (!stat.isFile()) return record(action, "failed", `brain_read: "${requested}" is a directory, not a file.`);

      let body: string;
      try {
        body = fs.readFileSync(absolute, "utf8");
      } catch (err) {
        return record(action, "failed", `brain_read could not read "${requested}": ${(err as Error).message}`);
      }
