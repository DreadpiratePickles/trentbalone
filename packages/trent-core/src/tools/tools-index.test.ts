...
describe("buildTrentToolAdapters", () => {
  ...
  it("every tool name this builder can produce is a reserved built-in name", async () => {
    ...
    expect(toolsetAdapters(adapters)).toEqual(["file_ops", "terminal", "web", "code_execution", "delegation", "cron", "skills", "plugins", "browser", "vision", "mcp", "human", "media", "social", "business", "a2a"]);
