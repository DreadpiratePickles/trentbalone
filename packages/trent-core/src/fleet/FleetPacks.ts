...
export const FLEET_PACKS: Record<string, FleetPack> = {
  ...
  social: {
    ...
    state:
      "Posts, scheduled posts, replies, the comment inbox and post numbers run through the social toolset once an account is connected: Bluesky directly (trent connect bluesky; up to four images or one clip from the workspace, with alt text) and any channel Buffer holds (X, LinkedIn, Threads, a Facebook Page; an image or a video only from a URL the owner hosts, since Buffer has no upload) today; Facebook and Instagram directly, and YouTube replies and numbers, when Meta or Google is connected and its review passes (the direct paths also need the app store). No DMs and no YouTube publishing. Nothing is published or replied to until the owner approves that exact call. The content and growth seats make the calls; the analyst seat has no social toolset and reads what they pull. Without a connection the calendar, posts and triage are written as files.",
