  ...
  switch (message.method) {
    case "initialize": {
      ...
      return response(id, {
        ...
        instructions:
          "Trent runs governed AI-company operations. Call trent_list_app_solo_options before App-Solo work, then use trent_run_agent with engine=solo; optionally pass appId to choose the seat-specific app. trent_run_agent returns runId plus nextCall for structured polling. Follow nextCall to trent_get_run and inspect evidenceSummary for verification, artifacts, commands, and preview proof. Approval gates, spend limits, RLS, and audit logging remain enforced inside Trent.",
      });
    }
