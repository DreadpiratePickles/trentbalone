...
import { normalizeMarketingAccountInput } from "@/lib/marketing/accounts";
...
export async function GET(request: Request) {
  ...
  return withRlsContext(companyId, async () => {
    ...
    const accounts = await store.listMarketingAccounts(company.id);
...
export async function POST(request: Request) {
  ...
  let input;
  try {
    input = normalizeMarketingAccountInput(body);
  } catch (error) {
  ...
  return withRlsContext(input.companyId, async () => {
    ...
    const account = await store.upsertMarketingAccount({ ...input, companyId: company.id });
