...
| 18 | App Memory Tiers | Reads `DATABASE_URL` the way the wrapped application does and reports which of the three cases this profile is in — unset or empty (reachable but in-process, so tier rows die with the process; the app's store is not loaded, because loading it constructs a Prisma client the profile would never query), a postgres URL (reachable and durable, measured with one document read through the app's own store), or a `file:` SQLite path (unreachable, recall falls back to run-derived candidates). See [configuration.md](configuration.md), "Company memory in the app" | no |
