...
import { ingestSocialInbox } from "@/lib/social/inbox-ingestion";
  ...
  return withRlsContext(companyId, async () => {
    ...
    try {
      const result = await ingestSocialInbox({
        ...
        socialAccountId: text(body.socialAccountId),
      ...
      return NextResponse.json(
        { error: error instanceof Error ? error.message : "Social inbox ingestion failed" },
        { status: 500 },
      );
    }
  });
