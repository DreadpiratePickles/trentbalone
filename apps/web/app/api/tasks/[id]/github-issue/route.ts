...
import { createGitHubIssueForTask } from "@/lib/github";
...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (task.agentRole !== "engineer") {
    return NextResponse.json({ error: "Only engineer tasks can create GitHub issues" }, { status: 400 });
  }

  if (!["queued", "running", "waiting_approval"].includes(task.status)) {
    return NextResponse.json(
      { error: "Task must be queued, running, or waiting for approval before execution" },
      { status: 409 }
    );
  }
