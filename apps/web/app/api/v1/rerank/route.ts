...
import { buildRerankResponse, normalizeRerankRequest } from "@/lib/ai-proxy/rerank";
import { checkRateLimit, rateLimitExceeded } from "@/lib/rate-limit";
import { withRlsContext } from "@/lib/with-rls";

export async function POST(request: Request) {
  const auth = await authenticateProxyApiKey(request.headers.get("authorization"));
  if (!auth || !auth.scopes.includes("rerank")) {
  ...
  let normalized;
  try {
    normalized = normalizeRerankRequest(body);
  } catch {
    return NextResponse.json({ error: "invalid_rerank_request" }, { status: 400 });
  }

  const quota = await assertProxyQuotaAvailable(auth, { tokens: `${normalized.query} ${normalized.documents.join(" ")}`.length });
  ...
  return withRlsContext(auth.companyId, async () => {
    const result = await buildRerankResponse({ companyId: auth.companyId, request: normalized });
