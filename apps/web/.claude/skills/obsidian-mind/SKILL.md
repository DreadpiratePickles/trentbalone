---
name: obsidian-mind
description: Work in the Obsidian Mind vault/template repo or any vault built from it. Use when editing its AGENTS/CLAUDE-guided structure, hooks, slash commands, notes, Bases, Canvases, or setup scripts.
---
...
Read the repo's `AGENTS.md` and `CLAUDE.md` before editing. They define the authoritative vault workflow, folder map, and update rules.
...
- Prefer the repo's own `.claude/commands/` and `.claude/scripts/` for lifecycle tasks.
- Treat `brain/`, `work/`, `perf/`, `org/`, `reference/`, `thinking/`, `templates/`, and `.claude/` as first-class surfaces.
